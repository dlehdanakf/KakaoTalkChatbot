<?php
	class DeliveryGroup extends BasicModel {
		...
		public function save() {
			$pdo = B::DB();
			$query = $pdo->prepare("INSERT INTO delivery_group (title, label, description, thumbnail_id) VALUE (:t, :l, :d, :i)");
			$query->execute([
				':t' => $this->title,
				':l' => $this->label,
				':d' => $this->description,
		...
		public function update(){
			$query = B::DB()->prepare("UPDATE delivery_group SET title = :t, label = :l, description = :d, thumbnail_id = :i WHERE id = :id");
			$query->execute([
				':t' => $this->title,
				':l' => $this->label,
				':d' => $this->description,
