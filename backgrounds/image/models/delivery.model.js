    ...
    async fetchDataFromDB(){
        ...
        try {
            ...
            try {
                ...
                const [ rows ] = await connection.query('SELECT * FROM delivery WHERE id = ?', [ this.id ]);
                connection.release();
                ...
                console.error('Delivery 모델 데이터를 가져오던 도중 오류가 발생했습니다.');
                connection.release();

                return false;
            }
        } catch(err) {
            console.error('Mysql database connection error occur while fetch delivery model');
            return false;
        }
    }
}
