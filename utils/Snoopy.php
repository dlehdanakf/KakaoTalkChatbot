...
class Snoopy
{
            ...
            case "http":
                ...
                if ($this->_connect($fp)) {
                    ...
                    if ($this->_framedepth < $this->maxframes && count($this->_frameurls) > 0) {
                        ...
                        while (list(, $frameurl) = $this->myEach($frameurls)) {
    ...
    function _striplinks($document)
    {
        ...
        while (list($key, $val) = $this->myEach($links[2])) {
            if (!empty($val))
                $match[] = $val;
        }

        while (list($key, $val) = $this->myEach($links[3])) {
            if (!empty($val))
                $match[] = $val;
        }
    ...
    function _httprequest($url, $fp, $URI, $http_method, $content_type = "", $body = "")
    {
        ...
        if (!empty($this->rawheaders)) {
            ...
            while (list($headerKey, $headerVal) = $this->myEach($this->rawheaders))
    ...
    function _prepare_post_body($formvars, $formfiles)
    {
        ...
        switch ($this->_submit_type) {
            case "application/x-www-form-urlencoded":
                reset($formvars);
                while (list($key, $val) = $this->myEach($formvars)) {
                    if (is_array($val) || is_object($val)) {
                        while (list($cur_key, $cur_val) = $this->myEach($val)) {
            ...
            case "multipart/form-data":
                ...
                reset($formfiles);
                while (list($field_name, $file_names) = $this->myEach($formfiles)) {
                    settype($file_names, "array");
                    while (list(, $file_name) = $this->myEach($file_names)) {
